...
public:
	...
	inline void set(T x, T y, T x2, T y2) { UpperLeftCorner.set(x,y); LowerRightCorner(x2,y2); }
	inline void set(const vector2d<T>& upperLeft, const vector2d<T>& lowerRight) { UpperLeftCorner = upperLeft; LowerRightCorner = lowerRight; };

	inline bool isEmpty() const { return UpperLeftCorner.X >= LowerRightCorner.X || UpperLeftCorner.Y >= LowerRightCorner.Y; }

	//
	inline rect& operator=(const rect<T>& other)
	{
		_ASSERT(this != &other);
		if (this != &other)
		{
