...
public : 
	...
	inline static pointer address(reference r) { return &r; }
	inline static const_pointer address(const_reference r) { return &r; }
	...
	inline static void construct(pointer p, const T& t) { new(p) T(t); }
	inline static void destroy(pointer p) { p->~T(); }

	template<typename U>
	qzone_allocator<U>& operator=(const qzone_allocator<U>& other) 
	{ 
		if (this != &ther)
		{
	...
	inline bool operator==(const qzone_allocator& a) const { return this==&a || (m_small == a.m_small && m_default == a.m_default); }
