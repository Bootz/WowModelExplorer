...
public:
	dbc::record getRecord(u32 idx) const
	{
		...
		return record(this, _recordStart + idx * RecordSize);
	}
	...
	{
		T_RecordSparseLookup::const_iterator itr = RecordSparseLookup.find(index);
		if (itr == RecordSparseLookup.end())
			return -1;
...
protected:
	...
	typedef std::map<u32, u32, std::less<u32>, qzone_allocator<std::pair<u32, u32>>> T_RecordSparseLookup;
	T_RecordSparseLookup		RecordSparseLookup;
};
...
{
public:
	itemDB(wowEnvironment* env) : dbc(env, "DBFilesClient\\Item.db2") { }
...
{
public:
	itemSparseDB(wowEnvironment* env) : dbc(env, "DBFilesClient\\Item-Sparse.db2") { }

	size_t getItemNameField() const
	{
		if (minorVersion >= 16309)
			return 100;
		else
			return 99;
	}
...
struct SItemRecord
{
	...
	s32 id, itemclass, subclass, type, model, sheath;
...
public:
	...
	void build(itemDB* itemDb,  itemSparseDB* itemSparseDb, itemDisplayDB* itemDisplay);
